...
from services.openai_client import summarize_with_openai
...
def call_llm_api(article_text, language):
    """
    Call the LLM API to generate a bullet point summarizing an article.
    Now uses OpenAI instead of Gemini with optimal 15-word format.
    ...
    try:
        ...
        result = summarize_with_openai(article_text, language)
        return result
...
def print_summary_points(data):
    ...
    """
    if 'bullet_point' in data:
        bullet_point = fix_unicode(data['bullet_point'])
        print(f"• {bullet_point}")
        
        if 'full_summary' in data:
            full_summary = fix_unicode(data['full_summary'])
            print(f"\nFull Summary: {full_summary}")

        print(f"\nRecommended tone: {data.get('tone', 'N/A')}")
